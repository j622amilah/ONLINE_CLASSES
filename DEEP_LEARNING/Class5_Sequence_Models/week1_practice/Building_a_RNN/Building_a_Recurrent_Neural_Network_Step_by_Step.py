...
def rnn_forward(x, a0, parameters):
    ...
    for t in range(T_x):
        ...
        a[:,:,t] = a_next  # size (n_a, m) which is a slice of a
        # Save the value of the prediction in y (≈1 line)
        y_pred[:,:,t] = yt_pred  # size (n_y, m) which is a slice of y_pred
...
def rnn_cell_backward(da_next, cache):
    ...
    inside = np.dot(Wax, xt) + np.dot(Waa, a_prev) + ba 
    dtanh = da_next * ( 1 - np.tanh(inside)*np.tanh(inside) ) # size (n_a, m)
    
    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = np.dot(Wax.T, dtanh)    # size (n_x, n_a)*(n_a, m) = (n_x, m)
    dWax = np.dot(dtanh, xt.T)    # size (n_a, m)*(m, n_x) = (n_a, n_x)

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.dot(Waa.T, dtanh) # size (n_a, n_a)*(n_a, m) = (n_a, m)
    dWaa = np.dot(dtanh, a_prev.T)    # size (n_a, m)*(m, n_a) = (n_a, n_a)

    # compute the gradient with respect to b (≈1 line)
    dba = np.sum(dtanh, axis=1, keepdims=True)  # sum across columns to get (n_a, 1)
...
def rnn_backward(da, caches):
    ...
    for t in reversed(range(T_x)):
        ...
        gradients = rnn_cell_backward(da0[:,:,t], caches[0])
