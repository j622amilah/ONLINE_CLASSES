...
def sample(parameters, char_to_ix, seed):
    ...
    """
    #A function that predicts a word based on the Weight matricies and dictionary list
    # ----------------------------
    ...
    n_a = Waa.shape[1]
    n_x = vocab_size     # Number of unique characters in loaded text
    n_y = parameters["Wya"].shape[0]
    m = 1     # Number of word examples
    # ----------------------------
    
    # ----------------------------
    # Initialize Forward propagation variables:
    ### START CODE HERE ###
    ...
    # Loop over each character until the end of the word is predicted
    counter = 0
    max_num_of_loops = 50  # The max number of time to let the loop run
    
    flag = 0    # Tells if y predicted the end of the word: flag=0 means the end of word was NOT found yet
    # flag=1 means the loop brakes because the end of the word was found
    
    
    while (flag  == 0) & (counter < max_num_of_loops+1):
    #while (idx != newline_character and counter != 50):
        # Forward propagation to obtain y
        ...
        idx = np.random.choice(list(range(n_y)), p = np.ravel(y))
        
        # Append the index to "indices"
        indices = indices + [idx]
        created_char = created_char + [ix_to_char[idx]]
        
        # Update flag based on EOS
        if idx == char_to_ix["\n"]:
            # Print out result
            ...
            # Update x
            x = np.zeros((n_x, 1))  # Reset to zero
            x[idx] = 1  # Make a one-hot vector meaning put a 1 where there was a 
            #print('x : ' + str(x))
...
def model(data_x, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        #X = np.zeros((n_x, 1))  #Y is the length of T_x
        #cout = 0
        #for i in range(n_x):
        ...
        #X = single_example_ix  #(WORKS, but it cuts the first character off for Y)
        ...
        # Way 4 : Putting None ONLY in the first entry of X, followed by the word entries 
        # without spaces in sequencial order (THE ANSWER)
...
def model(data_x, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        # Convert the list of characters to a list of integers
        #single_example_ix  = []
        #for i in range(0,len(single_example_chars)):
        #    single_example_ix = single_example_ix + [char_to_ix[single_example_chars[i]]]
        #print('Way 1 - single_example_ix : ' + str(single_example_ix))
        #OR
        single_example_ix = [char_to_ix[single_example_chars[i]] for i in range(len(single_example_chars))]
        #print('3. index of words : single_example_ix : ' + str(single_example_ix))
        # ----------------------------
